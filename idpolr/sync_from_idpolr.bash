...
start_prod_services() {
  ssh $remote_user@$remote_hostname "sudo service slapd starte
}
...
edit_dump() {
  sed -i "/structuralObjectClass:.*/d" $dump_path/$dump_name
  sed -i "/entryUUID:.*/d" $dump_path/$dump_name
  sed -i "/creatorsName:.*/d" $dump_path/$dump_name
  sed -i "/createTimestamp:.*/d" $dump_path/$dump_name
  sed -i "/entryCSN:.*/d" $dump_path/$dump_name
  sed -i "/modifiersName:.*/d" $dump_path/$dump_name
  sed -i "/modifyTimestamp:.*/d" $dump_path/$dump_name
  sed -i "/pwmGUID:.*/d" $dump_path/$dump_name
  sed -i "/pwdChangedTime:.*/d" $dump_path/$dump_name
  sed -i "/pwmLastPwdUpdate:.*/d" $dump_path/$dump_name
  sed -i "/pwdFailureTime:.*/d" $dump_path/$dump_name
}
...
import_slap() {
  ...
  sudo rm __db.*
  sudo rm *.bdb
  sudo slapadd -v -c -n 1 -l $dump_path/$dump_name
  ls -l
