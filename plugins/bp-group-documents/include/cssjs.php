...
function bp_group_documents_front_cssjs() {
    ...
    $bp_gr_subdirectory = end( explode( '/', BP_GROUP_DOCUMENTS_DIR ) );
    $bp_gr_url = plugins_url() . '/' . $bp_gr_subdirectory;

    //if we're on a group page
    if ($bp->current_component == $bp->groups->slug) {
        wp_enqueue_script('bp-group-documents', $bp_gr_url . '/js/general.js', array('jquery'), BP_GROUP_DOCUMENTS_VERSION);
        ...
        wp_register_style('bp-group-documents', site_url() .  $bp_gr_url . '/css/style.css', false, BP_GROUP_DOCUMENTS_VERSION);
        wp_enqueue_style('bp-group-documents');

        switch (BP_GROUP_DOCUMENTS_THEME_VERSION) {
            case '1.1':
                wp_enqueue_style('bp-group-documents-1.1',  $bp_gr_url . '/css/11.css');
                break;
            case '1.2':
...
function bp_group_documents_admin_cssjs() {
    wp_enqueue_style('bp-group-documents-admin', site_url() . '/' . BP_GROUP_DOCUMENTS_DIR . '/css/admin.css');
}
