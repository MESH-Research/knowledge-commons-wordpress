<?php
/*
Plugin Name: More Privacy Options (KC Fork)
Plugin URI:	http://wordpress.org/extend/plugins/more-privacy-options/
Version: 4.6-kc.1
Description: KC fork of More Privacy Options. Adds actual network membership verification for the "Network Users Only" privacy level using BuddyPress member types. Original plugin by D. Sader.
Author: D. Sader, Knowledge Commons
...
class DS_More_Privacy_Options {
	...
	function ds_feed_login() {
		...
		global $current_blog, $blog_id;
			...
			if ( is_wp_error( $user ) ) {
				...
				// "Network Users Only" — check actual network membership.
				if ( '-1' == $current_blog->public
					&& class_exists( 'Humanities_Commons' ) && function_exists( 'bp_get_member_type' ) ) {
					$network_id   = Humanities_Commons::$society_id;
					$member_types = bp_get_member_type( $user_id, false );
					if ( ! is_array( $member_types )
						|| ! in_array( strtolower( $network_id ), array_map( 'strtolower', $member_types ), true ) ) {
						$deny = true;
					}
				}
				// "Members Only"
	...
	function ds_users_authenticator () {
		...
		// Check actual network membership via BuddyPress member types.
		if ( class_exists( 'Humanities_Commons' ) && function_exists( 'bp_get_member_type' ) ) {
			$user_id      = get_current_user_id();
			$network_id   = Humanities_Commons::$society_id;
			$member_types = bp_get_member_type( $user_id, false );

			if ( is_array( $member_types ) ) {
				$member_types_lower = array_map( 'strtolower', $member_types );
				if ( in_array( strtolower( $network_id ), $member_types_lower, true ) ) {
					return;
				}
			}

			// Logged in but not a member of this network.
			if( is_feed() ) {
			...
			<form name="loginform" id="loginform" />
				<p><a href="<?php echo network_home_url(); ?>"><?php echo __('Click', $this->l10n_prefix).'</a>'. __(' to continue', $this->l10n_prefix); ?>.</p>
					<?php $this->registered_users_login_message(); ?>
			</form>
			</div>
			</body>
			</html>
