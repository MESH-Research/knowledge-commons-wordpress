...
//                    if(($mobile_css.attr('media') != 'all')) {
        ...
        {
            check_is_mobile();
            $( window ).on( 'resize',  check_is_mobile );
            check_has_item_nav();
            render_layout();
            mobile_carousel();
        }
...
//                    $('.message-check, #select-all-messages').addClass('styled').after('<strong></strong>');
        ...
        $( 'a.show-options' ).on( 'click',  function ( event ) {
        ...
        jq( "#message-type-select" ).on( 'change', 
            function () {
        ...
        jQuery( document ).on( "keydown", ".bb-label-name", function ( e ) {
            if ( e.keyCode == 13 ) {
                jQuery( ".bb-add-label-button" ).trigger( 'click' );
            }
        } );
...
//            $( 'body' ).unbind();
        ...
        /*--------------------------------------------------------------------------------------------------------
         ...
         --------------------------------------------------------------------------------------------------------*/
        $( '#group-create-body input[type=file], #avatar-upload input[type=file], #group-settings-form input[type=file]' ).on( 'change',  function ( e ) {
            var str = $( this ).val(),
                parts = str.split( '\\' ),
        ...
        if ( $( '.search-wrap input[type="text"]' ).hasClass( 'ui-autocomplete-input' ) ) {
            ...
            function searchWidth() {
                if ( !is_mobile ) {
                    ...
                    $( "#header-search .search-wrap" ).off( 'focus' );
                    $( "#header-search .search-wrap" ).off( 'focusout' );
                }
            }
...
//            $(window).resize(searchWidth);
        ...
        function attachClick() {
            $( '.more-items-btn' ).on( 'click',  function () {
        ...
        $window.on( 'resize',  attachMobileMenu );
        ...
        movePhotoButton();
        $window.on( 'resize',  movePhotoButton );
...
( function ( $ ) {
    $.fn.jRMenuMore = function ( widthfix ) {
        $( this ).each( function () {
            ...
            $( '#right-panel-inner' ).on( 'resize',  function () {
            ...
            function alignMenu( obj ) {
                ...
                $( obj ).find( "li.hideshow > a" ).on( 'click',  function ( e ) {
                    e.preventDefault();
