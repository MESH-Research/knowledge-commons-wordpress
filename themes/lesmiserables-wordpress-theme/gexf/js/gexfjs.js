...
var GexfJS = {
    ...
    i18n : {
        ...
        "en" : {
            ...
            "undirLinks" : "Meets with",
            "undirLinks2" : "Undirected links with:",
...
function displayNode(_nodeIndex, _recentre) {
    ...
    // Sources and targets for the node
    var edgesOfTheNode = [];
    ...
    // Get some parameters
    var enableSortingOptions = edgesOfTheNode.length > 2;
    ...
    if (_nodeIndex != -1) {
        ...
        _str += '<h2 class="node-name"><div class="largepill" style="background: ' + _d.color.base +'"></div>' + _d.label + '</h2>';

        //_str += '<h4>' + strLang("nodeAttr") + '</h4>';
        //_str += '<ul><li><b>id</b> : ' + _d.id + '</li>';

        var visibleAttributes = ["name", "description"];

        for (i in _d.attributes) {
            //_str += '<li><b>' + strLang(i) + '</b> : ' + replaceURLWithHyperlinks( _d.attributes[i] ) + '</li>';
            if(visibleAttributes.indexOf(i.toString()) > -1) {
                var label = "";
                ...
                _str += '<li><b>' + label + '</b> : ' + _d.attributes[i] + '</li>';
            }
        }

        var sortingMarkup = enableSortingOptions ? '<ul class="sort-options"><li data-sort-type="alphabet" class="sort-option active">sort by character</li><li data-sort-type="cluster" class="sort-option option-b">sort by community</li></ul>' : '';

        _str += '</ul><h4 class="links">' + ( GexfJS.graph.directed ? strLang("inLinks") : strLang("undirLinks") ) + sortingMarkup + '</h4><ul class="meets-with-list" id="meets-with-list">';



        // Default alphabetical sorting
        edgesOfTheNode.sort(function(a, b) {
            var nameA = a.node.label.toLowerCase(), nameB = b.node.label.toLowerCase();
            if(nameA < nameB) return -1;
            if(nameA > nameB) return 1;
            return 0;
        });

        // HTML list assembling
        for(var i = 0, l = edgesOfTheNode.length; i < l; i++) {
            var link = edgesOfTheNode[i].isTarget ? edgesOfTheNode[i].edge.source : edgesOfTheNode[i].edge.target,
                groupId = 'g' + parseInt(edgesOfTheNode[i].node.attributes.group);

            _str += '<li class="meets-with-list-item" data-cluster=' + groupId + '><div class="smallpill" style="background: ' + edgesOfTheNode[i].node.color.base +'"></div><a href="#" onmouseover="GexfJS.params.activeNode = ' + link + '" onclick="displayNode(' + link + ', true); return false;">' + edgesOfTheNode[i].node.label + '</a>' + ( GexfJS.params.showEdgeLabel && edgesOfTheNode[i].edge.label ? ' &ndash; ' + edgesOfTheNode[i].edge.label : '') + ( GexfJS.params.showEdgeWeight && edgesOfTheNode[i].edge.weight ? ' [' + edgesOfTheNode[i].edge.weight + ']' : '') + '</li>';
        }
        ...
        //}
        //
        ////if (GexfJS.graph.directed) _str += '</ul><h4>' + strLang("outLinks") + '</h4><ul class="links-list">';
        //
        //for (var i in GexfJS.graph.edgeList) {
        //    var _e = GexfJS.graph.edgeList[i];
        //    if ( _e.source == _nodeIndex ) {
        ...
        success: function(data) {
            ...
            $(_nodes).each( function() {
                ...
                _d.color = {
                    ...
                    base : "rgba(" + _r + "," + _g + "," + _b + "," + GexfJS.nodeOpacity + ")",
...
function traceMap() {
    ...
    // Set the stroke
    GexfJS.ctxGraphe.lineWidth = 1;
    //GexfJS.ctxGraphe.strokeStyle = "rgba(0,100,0,0.8)";
    GexfJS.ctxGraphe.strokeStyle = "rgba(0,0,0,0.20)";
    ...
    for (var i in GexfJS.graph.nodeList) {
        ...
        if (_d.visible && _d.withinFrame) {
            if (i != _centralNode) {
                ...
                //console.log(_d.color.base);
                //GexfJS.ctxGraphe.fillStyle = "rgba(0, 0, 0, .15)";
        ...
        if (_d.visible && _d.withinFrame) {
            if (i != _centralNode) {
                ...
                if (_fs > GexfJS.params.textDisplayThreshold) {
                    ...
                    GexfJS.ctxGraphe.font = Math.floor( _fs )+"px 'Alegreya SC', Arial, serif";
                    ...
                    GexfJS.ctxGraphe.fillText(_d.attributes.name, _d.coords.real.x, _d.coords.real.y); // Todo: decide what attribute to use
                    //GexfJS.ctxGraphe.fillStyle = "rgba(255, 255, 255, 1)";
                    //GexfJS.ctxGraphe.fillText(_d.attributes.name, _d.coords.real.x+5, _d.coords.real.y+5); // Todo: decide what attribute to use
                }
            }
        }
    }
    ...
    if (_centralNode != -1) {
        ...
        GexfJS.ctxGraphe.closePath();
        GexfJS.ctxGraphe.fill();
        GexfJS.ctxGraphe.strokeStyle = "rgba(0,0,0,0.9)";
        GexfJS.ctxGraphe.stroke();
        var _fs = Math.max(GexfJS.params.textDisplayThreshold + 2, _dnc.coords.real.r * _textSizeFactor) + 2;
        var _shift = 1;
        GexfJS.ctxGraphe.font = "bold " + Math.floor( _fs )*2+"px 'Alegreya SC', Arial, serif";
        GexfJS.ctxGraphe.textAlign = "center";
        GexfJS.ctxGraphe.textBaseline = "middle";
        GexfJS.ctxGraphe.fillStyle = "rgba(255, 255, 255, 1.00)";
        GexfJS.ctxGraphe.fillText(_dnc.label, _dnc.coords.real.x - _shift, _dnc.coords.real.y);
        GexfJS.ctxGraphe.fillText(_dnc.label, _dnc.coords.real.x + _shift, _dnc.coords.real.y);
        GexfJS.ctxGraphe.fillText(_dnc.label, _dnc.coords.real.x, _dnc.coords.real.y - _shift);
        GexfJS.ctxGraphe.fillText(_dnc.label, _dnc.coords.real.x, _dnc.coords.real.y + _shift);
    ...
    GexfJS.ctxMini.strokeStyle = "rgb(160, 160, 160)";
...
$(document).ready(function() {
    ...
    $('body').on('click', '.sort-option',function () {
        $('.sort-option').removeClass('active');
        $(this).addClass('active');

        var sort;

        switch ($(this).data('sort-type')) {
            case "alphabet":
                sort = function () {
                    tinysort('ul#meets-with-list>li', 'a')
                };
                break;

            case "cluster":
                sort = function () {
                    tinysort('ul#meets-with-list>li', {
                        attr: 'data-cluster'
                    })
                };
                break;
